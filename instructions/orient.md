...
Steps:
3. Are you inside the clone? Yes when `git rev-parse --show-toplevel` succeeds AND `git remote get-url origin` contains `protocolvision/workshop-kitkraft`. Never test the folder name. If yes, go to step 5.
...
6. Your folder. If `ROOT/participants/NAME/` does not exist: `cp -R ROOT/PARTICIPANT_TEMPLATE ROOT/participants/NAME`. Then `cd ROOT/participants/NAME`. In the report, tell the participant to open the harness inside that folder from now on (that is what makes the autolog fire and keeps you inside your own files).
7. Read `AGENTS.md` (in ROOT) fully. You follow it for every step in this workshop.
