...
Preconditions: `git pull --rebase origin main` succeeds; `<target>/WITHDREW.md` does not exist (if it does, tell the participant to pick another target); `participants/<name>/kit/` exists (v2 if there was one); `<target>/README.md` exists. If the target is missing, tell the participant to ask the owner to push, wait, pull again.
...
Report to the participant: the bridge file path, the count of unblock lines, whether it worked, and the paragraph, before pushing.
```
