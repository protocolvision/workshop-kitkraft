...
- `git pull --rebase origin BRANCH` succeeds; `participants/NAME/kit/` exists (v2 if there was one); `KIT/README.md` exists and the target folder has no `WITHDREW.md` (if it does, the target withdrew: switch to a facilitator kit and log a `decision` line). If the target is missing, tell the participant to ask the owner to push, wait, pull again; still missing after two minutes → switch to `facilitator/sachin/kit/` or `facilitator/rafa/kit/` and log the switch as a `decision` line.
