...
ABSENT variant (a group-mate is not in the room): tell the participant to swap so the two present read each other; when the absent kit appears in the repo later, run steps 3–4 on it and the participant posts your report in #workshop-kitkraft tagged to the owner. If the participant is the absent one: run steps 3–4 on the kit named in the pin and post the report the same way; log the reports about your own kit when they arrive.
