...
Failures: auth prompt → type nothing; `gh auth setup-git` by the participant, then continue. Push rejected after 3 tries → print the error; the participant posts it in #kitcraft and retries after the fix; nobody commits the file for them. Private kit → edit the content on the machine only; log the `progress` lines in the public LOG.md without content details.
