...
Steps:
1. Read every `blocker` line, from both tests. For each, decide which file fixes it. Independence (it could not run without the owner): a missing input or format → `kit/README.md` Inputs; a missing step, check or tacit rule → `kit/SKILL.md` Steps, Constraints or Do not; a wrong trigger → the `description` line in SKILL.md frontmatter; a missing example → `kit/examples/`. Fit (the output suited the owner, not the reader): a missing or vague `Adapts to:` line → `kit/README.md` `Adapts to:` (the context the person running it brings, and where it goes in); a step that hard-codes the owner's data, names or settings → `kit/SKILL.md` Steps, so it asks for or reads the user's context at run time. Say your plan in one line per blocker and wait for the participant to say "go" or change it.
