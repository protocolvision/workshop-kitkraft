...
Human header: Block 1 writes `participants/<name>/s1-inventory/inventory.md` from your answers and pushes it; Block 2 pulls the others' folders and names two kits that could complement yours to yours. Your harness drafts the list for you: it reads its own memory of you, this conversation, and the AI setup on your machine (skills, saved prompts, commands, project instructions, prompt history), finds three to ten tools and kits you have used or built for repeated use, and shows you the draft. You confirm, correct or cut each one before anything is pushed. It lists only what it finds evidence for; it never invents a kit. The file shape is in `participants/README.md`. Read other people's inventories only through your agent, never in the browser.
