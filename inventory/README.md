...
Preconditions: `git rev-parse --show-toplevel` ends in `workshop-kitkraft` (else stop: reopen the harness inside the clone); `git pull --rebase origin main` succeeds (auth prompt → `gh auth login`, `gh auth setup-git`, continue).
...
1. Ask the participant, one question at a time, and write the answers into `inventory/<name>.md` using the headings of `inventory/_example-rafa.md`:
   ...
   - `## Log`: append `<ISO timestamp> | S1 | <name> | progress | inventory written by harness`.
