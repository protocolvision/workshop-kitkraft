...
One file per participant: `participants/<your-name>/inventory.md`. Paste the block below into your harness (open inside the clone) and say "do this". Rule: read other people's inventories only through your agent, never in the browser.
...
Steps:
1. Ask the participant, one question at a time, and write the answers into `participants/<name>/inventory.md` using the headings of `participants/_example/inventory.md`:
...
3. Pull again. Read every other `participants/*/inventory.md` (not `_example-rafa.md`). Tell the participant the two entries closest to theirs and why, in three lines. Append `<ISO timestamp> | S1 | <name> | memory | closest: <a>, <b> — <why>` to their file's Log; commit `S1: <name>: closest entries`, pull --rebase, push.
