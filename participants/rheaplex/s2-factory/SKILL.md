...
4. **Compile the tokens.** Name the categories this medium actually has, then turn each row into named tokens under them. For a page or a screen that is usually type, ground, ink, one signal colour, grid, motion; for a drawing series, mark, line weight, ground, palette, growth, seed-to-seed variation; for other media, work them out from the medium and say what you chose. Do not carry a category the medium does not have. Give each category to the row whose "what to take" governs it, and label the token groups with that row's layer so the mapping is visible. A category with nothing behind it in the table stays empty rather than being filled in. Every token traces to a row; if one does not, drop it or add the row. Derive each value from that row's "what to take", in the medium's own units, and mark every value provisional: first-run numbers are a starting point that step 8 corrects, so write them to be overruled, not defended. A constraint that has no value form (no build step, no framework) is not a token — carry it into Do not instead. Hands on: the token block, in the medium's own form (CSS variables, a prompt frame, a theme file).
5. **Settle layout, or mark it discovered.** Layout is specified or discovered, never assumed. Specified: the person gave measures — column set, grid, canvas, safe margins, reading order — so write them as tokens beside the rest. Discovered: nobody has, so record `Layout: discovered` with the questions the build will answer (how many columns, how wide a label runs, where a rule goes), build them in step 6, and write the values that survived step 7 back into the spec. Never quietly invent a measure and present it as settled. Hands on: layout tokens, or the list of open measures.
6. **Write the terms** (generative or repeated work, or whenever the person names terms they want defined; otherwise skip and say so). One term per recurring feature: a definition of two to four sentences, two or three examples, two or three anti-examples, and a version number carried in its heading as `(v<n>)`. The definition text is what gets handed to a model, so write it for that reader. Hands on: the terms.
7. **Build one slice.** Build the smallest real thing the tokens describe — one page, one plate, one slide. Offer options where the tokens do not decide; keep every change small and reversible. Hands on: the slice, plus the questions the tokens could not answer.
8. **Review with the person.** They look at the slice and give the verdict; you do not. Record which references they cite while judging, and any token they overrule. Hands on: their verdicts and citations.
9. **Promote and emit.** A reference cited repeatedly in step 8 becomes a touchstone, with what it is a touchstone for. An overruled token is rewritten. A changed definition bumps the `(v<n>)` in its heading, and a changed spec bumps the file's own `Version:`. Write `aesthetic-spec.md` to the path from step 1, in this shape:
   ...
   ## Layout           `specified` with its measures as tokens, or `discovered` with the open questions
