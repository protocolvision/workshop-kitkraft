# Log — append-only, written by the harness
| time | session | name | type | note |
|---|---|---|---|---|
| <timestamp> | <session> | <participant> | <type> | <one line> |
| 2026-09-20T20:07:59Z | async | kmr091 | progress | first push by harness |
| 2026-09-21T16:37:37Z | S1 | kmr091 | progress | inventory written by harness |
| 2026-09-21T16:40:44Z | S1 | kmr091 | memory | related: sachben91/Session rituals, mjmarte/Manuscript production line — carry-forward baseline for daily changes; explicit claim-verification gate |
| 2026-09-21T16:47:09Z | async | kmr091 | decision | S2 source: own: kit 1, policy research kit |
| 2026-09-21T16:50:08Z | async | kmr091 | progress | Job line in s2-factory README rewritten by participant |
| 2026-09-22T08:41:02Z | S2 | kmr091 | progress | factory from inventory: rung 1; SKILL, example |
| 2026-09-22T09:05:00Z | async | kmr091 | blocker | Step 2 doesn't say what to do if web search isn't available or turns up thin coverage on a niche topic |
| 2026-09-22T09:05:00Z | async | kmr091 | blocker | "Current public sources" isn't defined — no guidance on recency cutoff, how to weigh source types against each other, or when the search has covered enough ground to stop |
| 2026-09-22T09:05:00Z | async | kmr091 | blocker | Adapts to: names policy area, sub-topic, baseline but doesn't say what to supply for a policy area unlike the example (non-AI, non-UK, slower-moving or sparser public coverage) |
| 2026-09-22T09:15:00Z | async | kmr091 | progress | v2: SKILL.md step 2 now says what to do if search is unavailable or thin (ask for what the person has, flag partial coverage) (fixes: Step 2 doesn't say what to do if web search isn't available or turns up thin coverage on a niche topic) |
| 2026-09-22T09:15:00Z | async | kmr091 | progress | v2: SKILL.md Constraints now defines "current" against the baseline, weighs government/company sources over secondary commentary, and gives a stopping signal for search (fixes: "Current public sources" isn't defined) |
| 2026-09-22T09:15:00Z | async | kmr091 | progress | v2: README Adapts to: now states the process holds for any policy area and names what changes on a slower-moving or sparser topic (fixes: Adapts to: doesn't say what to supply for a policy area unlike the example) |
| 2026-09-22T09:15:00Z | async | kmr091 | memory | the participant knew the kit should still work on a niche or slow-moving topic and with search unavailable; the kit did not say so until v2 |
| 2026-09-22T16:04:20Z | S3 | kmr091 | progress | bridge to patwater: read RAG Factory README + SKILL.md + examples; kind confirmed factory-factory |
| 2026-09-22T16:10:07Z | S3 | kmr091 | unblock | corpus size/structure supplied by kmr091: small fixed core (gov.uk AI pages, UK Parliament/Hansard, EUR-Lex/EC digital strategy, AI-related only) plus open-ended per-run news/company search with no fixed list |
| 2026-09-22T16:10:27Z | S3 | kmr091 | progress | bridge to patwater: ran RAG Factory steps 1-5 on my corpus/goal; typology read-only; filled pipeline spec (hybrid indexed core + live-search layer) |
| 2026-09-22T17:01:58Z | async | kmr091 | progress | S4 show-and-tell written from LOG, factory README, bridge log |
