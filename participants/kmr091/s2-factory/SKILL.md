---
name: kmr091
description: Produces a dated, sourced policy developments briefing — what has changed on a policy sub-topic since a given baseline — plus, if asked, a first-take view on a development or proposal. Use when the user wants to get up to speed on a policy area, wants to know what has changed since they last checked, or wants a quick take on a development or proposal in that area.
---

## When to use
When I need a scheduled or event-triggered update on a policy area — either a routine check, or something triggered by a known event like a company releasing a new tool — and want to understand what has changed and what it means for real-time policy decisions.
