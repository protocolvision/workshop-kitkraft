...
Each participant pastes Block 1 of `instructions/S1-inventory.md` into their harness. It writes `participants/<name>/s1-inventory/inventory.md` in the one schema (three to ten AI tools and kits they use repeatedly, drafted from the harness's memory and the participant's AI setup on their machine, each with what it is, the job, how often, what they adapted, who else uses it and where it lives; the kit to convert; posture; log), commits, pushes with the retry rule, verifies. What counts as a kit is read aloud: any AI tool or setup you use again and again to do a real job — a skill, a saved or reused prompt, a custom GPT or Claude project, a prompt document you copy from, an MCP server or plugin, a spreadsheet or document with an AI step, an automation with an AI step. The harness drafts from evidence only, never invents a kit, a field or an example, asks for what it cannot find, and pushes nothing until the participant confirms the draft. At minute 13 of the exercise, Block 2: pull, read every other inventory (skipping `_*` folders), name two kits that could complement theirs by what their kits are made of and the job they do, log them.
...
`PARTICIPANT_TEMPLATE/s1-inventory/inventory.md` — Kits I use (three to ten, six fields each, or `none yet`) · Kit to convert · Posture. No log section; lines go to `LOG.md`.
...
- Inventory is drafted, not interviewed: the harness reads its memory of the participant and their AI setup on the machine (skills, prompts, commands, project instructions, history), drafts three to ten kits used or built for repeated use, and the participant confirms before the push; replaces one to three by interview. (Rafa, Sep 21)
