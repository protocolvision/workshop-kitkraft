...
**Async 1 (Mon 16:30–20:00 UTC, three hours)** Paste `instructions/async-1.md`: choose the kit to convert in S2 — your own (the `## Kit` task or a `## Built / use` item) or a facilitator kit (`facilitator/rafa/s2-factory/`, `facilitator/sachin/s2-factory/`; copied into your folder with a `Source:` line) — and draft the README's Job and Interface lines. The participant rewrites the Job line in their own words. Nothing else; the gap is short and 1A sits inside it.
...
`PARTICIPANT_TEMPLATE/s1-inventory/inventory.md` — Recurring tasks (three, each with the two tests) · Built / use · Kit (the task) · Posture. No log section; lines go to `LOG.md`.
