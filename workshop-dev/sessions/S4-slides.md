...
- Rafa: stated Tuesday 15:43, held since — *a bridge is a protocol between two factories: the smallest thing both sides agree not to change*
- Sachin: for each paragraph, supports · contradicts · silent (table on screen, grouped by bridge kind; the hypothesis says "two factories", so the kit → kit rows are where it is tested hardest)
