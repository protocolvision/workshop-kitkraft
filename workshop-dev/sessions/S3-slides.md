...
Facilitation split: Rafa opens (slides 1–2: opener, what S2 left in the repo), briefs the exercise, runs the rooms, the reconvene, the check and the close, and keeps #kitcraft, breakouts, recorder, repo and the screen share. Sachin presents the theory (slides 3–8: F2F, the three kinds of bridge and the ladder, water-rate pipeline, Prime Radiant, trust, the hypothesis statement), starting right after Rafa's first two minutes. Breakout groups per the S3 pin (`../discord-pins.md`), one per room, breakout-1 … breakout-5. Examples 14, exercise brief 1 (rotation is pinned beforehand), breakout 40 (15:45–16:25, Rafa's window), close 5. Rotation and posts: `../breakout-instructions.md`; the block participants paste: `instructions/S3-bridge.md`; these slides repeat them, they do not replace them.
...
> Note: one minute. Say it twice. Rafa pins it and hands over for the briefing.
