...
- Say the failed step in one sentence (auth, wrong clone, wrong branch, push rejected, harness opened outside the clone); Rafa opens a #kitcraft thread per person for pasted errors.
...
7. Conflict check first: `git grep -l '^<<<<<<<' -- participants/NAME` must print nothing; if it does, fix the file before committing. Push. `git add participants/NAME && git commit -m "S1: NAME: 1A push" && git pull --rebase origin BRANCH && git push origin HEAD:BRANCH`. Proof: `git fetch origin && git log origin/BRANCH --oneline -1 -- participants/NAME/LOG.md` shows "1A push". Then tell the participant to say "pushed" in #kitcraft.
