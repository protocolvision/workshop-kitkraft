...
|---|---|---|---|---|---|---|---|
| 20:00 | 00 | Sachin | listens | Slide 1; consent one-liner; "Recording on (KITCRAFT-S2)" in #kitcraft | harness open inside clone | — | `/record channel:#kafka name:KITCRAFT-S2` |
...
| 20:15 | 15 | Rafa | Slide 5: build briefing; starts the clock; time calls at 5, 10 | pulls; helps anyone with a new harness or a stub, in text | agent fills README / SKILL / examples from `kits/README.md` | `kits/<name>/` v1 | on; a facilitator speaks every ≤15 min |
...
| 20:41 | 41 | — | swaps rooms | swaps rooms | reports read aloud; owners log `blocker` lines | `kits/<name>/LOG.md` | off |
...
| 20:49 | 49 | Sachin | listens | Slide 7: round, pinned order; captures sentences | speak (≈40 s each) | `transcripts/S2/awareness-check.md` | on |
...
| 20:58 | 58 | Sachin | — | "Recording off"; reminds anyone async of the path | leave / stay in #kitcraft | — | `/stop` at 21:00 |

Post-session (Sachin, by 21:30): both S2 segments + summaries → `transcripts/S2/transcript.md`, `summary.md`, `awareness-check.md`; commit `S2: sachin: transcript + summary`. Check one `blocker` line per present participant's `LOG.md`; ping anyone missing. Overnight: watch for async pushes; ping their assigned readers.

Fallbacks: harness cannot push → paste in #kitcraft, Sachin commits under their name. Recorder does not restart at 20:48 → Sachin retries once, then the close is reconstructed from #kitcraft posts. Kit not pushed by 20:35 → read anyway from whatever is on `main`.
