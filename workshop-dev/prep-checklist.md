...
| Sep 17 | Dry run: clone, open harness, one autolog line, one push; `/record` + `/stop` in #kafka; one breakout move | both | |
| Sep 19 | Push check per registrant; 1A invites; decide post-workshop minimum for consent notice | Sachin | |
| Sep 21 | S1 + 1A; overnight: adjacency pairs from inventories → #kitcraft | both | |
| Sep 22 | S2–S4; re-run `/record` after each breakout; transcripts + SYNTHESIS.md into repo; portal regenerated | both | |
