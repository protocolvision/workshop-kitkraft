...
**Bijker, Wiebe E. *Of Bicycles, Bakelites, and Bulbs* (1995)** — closure and stabilization: how a technology's interpretive flexibility ends and one form becomes the product. S1 theory (kits before closure); in S2 the reason not to close too early — see §3a.
...
1. *Externalize — write down what you actually do.*
   - Taylor (1911) — the instruction card handed to each worker is SKILL.md's ancestor: the procedure taken out of the craftsman's head and put on paper. Cite for the move, not the politics.
   - Polanyi (1966) — "we can know more than we can tell": why the critique finds gaps, and why they are tacit rather than careless.
   - Nonaka & Takeuchi (1995) — tacit-to-explicit conversion as the step organizations fail at; S2's build is one person's externalization.
2. *Split — the steps become workstations.*
   - Smith (1776), Book I ch. 1 — the pin factory: dividing one job into distinct operations makes each step learnable and checkable. SKILL.md's numbered steps.
3. *Name the output — what leaves is the intermediate.*
   - Chandler (1977) — throughput and the coordination of flows between units; the thing handed on, not the machine, is what the organization manages. Also S2 slide 3: the scarce thing is coordination.
4. *Declare — publish what others may depend on.*
   - Hounshell (1984) — interchangeable parts depended on gauges rather than on each fitter's skill; the gauge is the published interface. The README's `Exports:` / `Interface:` / `Adapts to:` lines.
   - Baldwin & Clark (2000) — visible design rules versus hidden parameters: publish few rules, keep the rest free to change.
5. *Test cold — someone else runs it without you.*
   - The S2 critique itself (independence and fit); Polanyi (1966) predicts where it fails.
6. *Stabilize — hold the way in still, keep the inside free.*
   - Parnas (1972) — decompose by what is likely to change and hide it behind a stable interface; the rationale for rung one.

**The tensions**
- *Reproduction vs personalization → postponement.* Feitzinger & Lee (1997) — HP built printers identically and localized them (power supply, manuals) at regional distribution centres; customization pushed to the last step. Pine (1993) — mass customization as the general strategy. In the deck: SKILL.md's steps stay fixed, the user's context enters at run time.
- *Composability → fork versus compose.* McIlroy, Pinson & Tague (1978) — "expect the output of every program to become the input to another, as yet unknown, program": Unix tools compose by agreeing on one intermediate, a text stream. Forking (S2) copies and bends; composing (S3) consumes the output unchanged.
- *Why the gains lag → from shafts to wires.* Devine (1983) — group drive (one motor on the old line shafts) versus unit drive (a motor per machine), and why only the latter let the floor be laid out by workflow. David (1990) — the dynamo as the precedent for the productivity paradox; gains arrive after reorganization. S2 slide 2.
- *The destination is a job shop, not an assembly line.* Sabel & Zeitlin (1985) — flexible specialization as a durable alternative to mass production; backs Rao's "flexible job shop" so nobody pictures River Rouge.
- *Do not close too early.* Bijker (1995) — closure and stabilization end interpretive flexibility; David (1985) — QWERTY as lock-in of whatever was there first. Why rung one is the correct rung tonight.
- *The scarce thing is coordination, not machinery.* Chandler (1977) on throughput; Hughes (1983) — the reverse salient, the lagging component that holds a whole system back; in AI adoption it is the interface between practices, not model capability. Why Rao and Dixon succeeded as two non-coders.
...
| Factory → tool | **MCP** (2026-07-28 spec: Streamable HTTP, OAuth 2.1) · https://modelcontextprotocol.io/specification/2026-07-28 | C3PO exposes one |
...
Feitzinger, Edward, and Hau L. Lee. "Mass Customization at Hewlett-Packard: The Power of Postponement." *Harvard Business Review* 75, no. 1 (1997): 116–21.
...
McIlroy, M. D., E. N. Pinson, and B. A. Tague. "UNIX Time-Sharing System: Foreword." *Bell System Technical Journal* 57, no. 6 (1978): 1899–1904.

Nonaka, Ikujiro, and Hirotaka Takeuchi. *The Knowledge-Creating Company*. New York: Oxford University Press, 1995.

Parnas, D. L. "On the Criteria to Be Used in Decomposing Systems into Modules." *Communications of the ACM* 15, no. 12 (1972): 1053–58.
