...
Targets: the kit named in the pinned S3 table (the one you did not critique), or a facilitator kit (`facilitator/rafa/kit/`, `facilitator/sachin/kit/`). A target that has not pushed, or a stub whose owner is not online, means a facilitator kit, with the substitution logged as a `decision` line.
